...
Plugin URI: http://thisismyurl.com/plugins/auto-copyright-1/
Description: Automates the copyright notice for web sites.
...
function thisismyurl_autocopyright( $format_result=NULL ) {
	...
	$all_posts = get_posts( 'post_status=publish&order=ASC&numberposts=1' );
	$from = get_the_time( 'Y', $all_posts[0]->ID );

	$all_posts = get_posts( 'post_status=publish&order=DESC&numberposts=1' );
	$to = get_the_time( 'Y', $all_posts[0]->ID );
